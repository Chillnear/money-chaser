...
# ---- P5.9b: funding_carry override ----


def _make_wide_universe_snapshot() -> list[dict]:
    """พูล 5 เหรียญผ่าน screening filter (volume/OI) ทั้งหมด — BTC funding สุดขั้วบวก (0.001) ส่วนอีก 4
    เหรียญ funding=0.0 เท่ากันหมด กัน BTC ได้ percentile 1.0 (deviation 0.5 >= threshold 0.4 ของ
    decide_funding_carry) ในพูลที่ใหญ่พอ (>= MIN_POOL_SIZE_FOR_FUNDING_OVERRIDE=5) ต่างจาก hl_client
    fixture ปกติที่มีแค่ 2 เหรียญ (เจตนาให้ override ไม่ทำงานในเทสต์อื่นๆทั้งหมดของไฟล์นี้)
    """
    base = {"open_interest_usd": 5.0e7, "day_volume_usd": 1.0e8, "mark_px": 100.0, "prev_day_px": 100.0}
    return [
        {"coin": "BTC", "funding": 0.001, **base},
        {"coin": "PAXG", "funding": 0.0, **base},
        {"coin": "ETH", "funding": 0.0, **base},
        {"coin": "SOL", "funding": 0.0, **base},
        {"coin": "XRP", "funding": 0.0, **base},
    ]


@pytest.fixture
def wide_hl_client():
    universe = _make_wide_universe_snapshot()
    candles = {
        "BTC": _make_candles(n=60, start=80.0, daily_pct=1.5),  # เทรนด์ขึ้นแรง -> ติด shortlist แน่นอน
        "PAXG": _make_candles(n=60, start=100.0, daily_pct=0.02),
        "ETH": _make_candles(n=60, start=100.0, daily_pct=0.02),
        "SOL": _make_candles(n=60, start=100.0, daily_pct=0.02),
        "XRP": _make_candles(n=60, start=100.0, daily_pct=0.02),
    }
    return FakeHLClient(universe, candles)


def test_run_daily_pipeline_uses_funding_carry_override_when_pool_wide_and_funding_extreme(tmp_path, settings, wide_hl_client):
    # ห้ามเรียก LLM เลยรอบนี้ — ถ้าเรียกจะ error ทันที (ยืนยันว่า override ตัดสินใจเองไม่ต้องรอ AI)
    llm_client = _make_llm_client(AssertionError("ไม่ควรเรียก LLM ตอน funding_carry override ทำงาน"))
    broker = PaperBroker(starting_equity_usd=28.0, taker_fee_pct=0.045, slippage_pct=0.05)
    journal_dir = tmp_path / "journal"

    result = run_daily_pipeline(
        settings=settings, hl_client=wide_hl_client, llm_client=llm_client, broker=broker,
        model_registry=REGISTRY, today_date="2026-07-27", now_ts=time.time(),
        journal_dir=journal_dir, kill_path=tmp_path / "KILL", last_run_path=tmp_path / "last_run.json",
        starting_equity_usd=28.0,
    )

    assert result.action_taken == "opened_short"  # funding บวกสุดขั้ว -> short เก็บ funding
    saved_state = json.loads((journal_dir / "state.json").read_text(encoding="utf-8"))
    assert saved_state["open_position"]["asset"] == "BTC"
    assert saved_state["open_position"]["side"] == "short"

    decisions = [json.loads(line) for line in (journal_dir / "decisions.jsonl").read_text(encoding="utf-8").splitlines()]
    assert decisions[-1]["source"] == "funding_carry_override"


def test_run_daily_pipeline_does_not_override_when_pool_too_small(tmp_path, settings, hl_client):
    # hl_client fixture มาตรฐานมีแค่ 2 เหรียญ (BTC funding=0.0001, PAXG funding=0.0) — ต่างกันจริง แต่
    # pool_size=2 < MIN_POOL_SIZE_FOR_FUNDING_OVERRIDE=5 จึงต้องไม่ override ทำงานปกติผ่าน LLM path
    responses = [_fake_llm_response(_analyst_json())] * 4 + [_fake_llm_response(_judge_json())]
    llm_client = _make_llm_client(responses)
    broker = PaperBroker(starting_equity_usd=28.0, taker_fee_pct=0.045, slippage_pct=0.05)
    journal_dir = tmp_path / "journal"
